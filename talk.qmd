...
## {background-image="https://aws-obg-image-lb-1.tcl.com/content/dam/brandsite/region/in/blog/pc/detail/blog-march/washing-machine-dimensions/thumbnail.jpg" background-opacity=0.1 .center}
...
![Assistant Professor, Department of Health Outcomes Research and Policy](https://pharmacy.auburn.edu/directory/images/matthew-loop.jpg)
...
2. List reasons why you might want to use a Bayesian model over a Frequentist model
...
![](https://www.psychologicalscience.org/redesign/wp-content/uploads/2013/12/PAFF_121113_aggressionglucoseMFB_newsfeature.jpg)

```{r}
#| output: false
#| echo: true
f_bayes_skeptic <- stan_lm(gene_expression ~ experimental_group, 
        data = df,
        prior = R2(0.05, what = "mean"))

post_skeptic <- as_tibble(f_bayes_skeptic)
f_bayes_tb <- stan_lm(gene_expression ~ experimental_group, 
        data = df,
        prior = R2(0.9, what = "mean"))

post_tb <- as_tibble(f_bayes_tb)
```
...
```{r}
ggplot() +
  geom_density(data = c, aes(x = `experimental_groupwild type`), fill = au_colors[1], alpha = 0.5) +
  geom_density(data = post_skeptic, aes(x = `experimental_groupwild type`), fill = au_colors[2], alpha = 0.5) +
  geom_density(data = post_tb, aes(x = `experimental_groupwild type`), fill = au_colors[3], alpha = 0.5) +
  annotate(geom = "text", x = 3, y = 0.6, label = "Agnostic", color = au_colors[1]) +
  annotate(geom = "text", x = 3, y = 0.5, label = "Skeptic", color = au_colors[2]) +
  annotate(geom = "text", x = 3, y = 0.4, label = "True believer", color = au_colors[3])
```
